document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function safeFetch(url, opts = {}) {
    ...
    if (body && typeof body === "object" && body.code === 403) {
  ...
  try {
    const reservas = await safeFetch(
      "https://conexao-alimentar.onrender.com/reservas/minhas-reservas",
      { method: "GET" }
    );
    ...
    reservas.forEach(reserva => {
      ...
      const dataExpiracaoMs = reserva.dataExpiracao
        ...
        : (reserva.dataReserva && reserva.segundosTotais
          ? (parseDateTimeToMs(reserva.dataReserva) + (reserva.segundosTotais * 1000))
          : null);
    ...
    if (err.status === 403) {
      showToast("Permissão negada. Faça login com uma conta ONG.", "error");
    } else {
