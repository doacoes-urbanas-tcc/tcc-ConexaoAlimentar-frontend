document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (!response.ok) {
      addToast(`Erro ao carregar doações: ${response.status}`, "error");
      return;
    }
    ...
    doacoesFiltradas.forEach(doacao => {
      ...
      item.querySelector(".reservar-btn").addEventListener("click", () => {
...
async function verificarAvaliacoesPendentes(token) {
  try {
    ...
    if (reservasPendentes.length > 0) {
      ...
      addToast("Você precisa concluir suas avaliações antes de continuar.", "error");
      setTimeout(() => {
    ...
    addToast("Erro ao verificar avaliações pendentes", "error");
    return true;
  }
}
...
function addToast(message, type = "success") {
  ...
  toast.className = `
    px-4 py-2 rounded shadow text-white transition transform duration-300 ease-in-out
